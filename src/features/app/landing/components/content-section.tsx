    ...
    <section className={cn('space-y-6', className)} {...props}>
      <div className="flex items-center justify-between gap-4">
        <div className="flex max-w-[61.25rem] flex-1 flex-col gap-0.5">
          <h2 className="font-bold text-2xl leading-[1.1] md:text-3xl">
            {title}
          </h2>
          {description ? (
            <p className="max-w-[46.875rem] text-balance text-muted-foreground text-sm leading-normal sm:text-base sm:leading-7">
              {description}
            </p>
          ) : null}
        </div>
      ...
      <div className="space-y-8">
        <ChildrenShell
          className={cn(
            !asChild &&
              'grid xs:grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4',
          )}
        >
          {children}
        </ChildrenShell>
